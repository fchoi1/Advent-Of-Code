...
class Hill():
    ...
    def getStart(self) -> Tuple[int, int]:
        return next(((x, y) for y, row in enumerate(self.inputData) for x, value in enumerate(row) if value == 'S'), (-1, -1))

    def getEnd(self) -> Tuple[int, int]:
        return next(((x, y) for y, row in enumerate(self.inputData) for x, value in enumerate(row) if value == 'E'), (-1, -1))
    ...
    def solve(self, goal: str, useStart: Optional[bool] = True) -> int:
        ...
        while self.q:
            ...
            for (currentX, currentY) in tempQ:
                ...
                nextQ = self.checkAllowPath((currentX, currentY), elevation, nextQ, useStart)
            self.q = nextQ
            self.step += 1
        return -1

    def checkAllowPath(self, current: Tuple, elevation: str, nextQ: List, fromStart) -> List[Tuple[int, int]]:
        ...
        for y, x in directions:
            ...
            if not (0 <= temp[1] < self.length) or not (0 <= temp[0] < self.width) or (temp in self.visited):
                continue
