...
def parse_program(lines: Iterable[str]) -> tuple[Program, Registers]:
    ...
    program: Program = list(
        map(int, next(line_iter).removeprefix("Program: ").split(","))
    )
    # Instructions are between 0 and 7
    assert all(0 <= v <= 7 for v in program), "Program contains invalid instructions!"
...
print(aoc2024_day17_part2(data))
