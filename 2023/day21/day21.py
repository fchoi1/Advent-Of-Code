...
class StepCounter:
    ...
    def bfs(self, start: Tuple[int], target: Optional[int] = None) -> int:
        count, step = 0, 0
        q = [start]
        seen = set()
        while q:
    ...
    def getSteps2(self) -> int:
        ...
        # even case
        bs = 3 * size // 2
        big_tl = self.bfs((size - 1, size), bs)
        big_tr = self.bfs((-1, size - 1), bs)
        big_bl = self.bfs((size - 1, -1), bs)
        big_br = self.bfs((-1, 0), bs)
        ...
        return (
            ...
            + even * evenCount
            + (gridWidth - 1)
            * (big_tl + big_tr + big_bl + big_br - 4)  # start from outside box
